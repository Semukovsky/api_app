...
from PyQt6.QtWidgets import QMainWindow, QApplication, QLabel, QButtonGroup
...
class YandexMap(QMainWindow, Ui_MainWindow):
    ...
    def keyPressEvent(self, event):
        try:
            ...
            if key == Qt.Key.Key_PageUp:  # PGUP
                self.delta += 0.004
            elif key == Qt.Key.Key_PageDown:  # PGDN
                self.delta -= 0.004 if self.delta - 0.004 >= 0 else 0
            elif key == Qt.Key.Key_Up:  # UP
                self.coords = self.moving('UP')
            elif key == Qt.Key.Key_Down:  # DOWN
                self.coords = self.moving('DOWN')
            elif key == Qt.Key.Key_Left:  # LEFT
                self.coords = self.moving('LEFT')
            elif key == Qt.Key.Key_Right:  # RIGHT
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.buttons() == Qt.MouseButton.LeftButton:
            if 200 <= x <= 800 and 0 <= y <= 600:
                delta_x_y = ((x - 500) * 2.5, -y + 300)
